...
{
    public static class BuiltinBloomTweenExtensions
    {
        static readonly TweenGetter<Bloom, float4> colorGetter = self =>
        {
            var color = self.color.value;
            return UnsafeUtility.As<Color, float4>(ref color);
        };
        static readonly TweenSetter<Bloom, float4> colorSetter = (self, x) =>
        {
        ...
        public static Tween<float4, NoOptions> TweenColor(this Bloom self, Color endValue, float duration)
        {
            Debugger.LogWarningIfPostProcessingInactive(self);
            return Tween.To(self, colorGetter, colorSetter, UnsafeUtility.As<Color, float4>(ref endValue), duration);
        }

        public static Tween<float4, NoOptions> TweenColor(this Bloom self, Color startValue, Color endValue, float duration)
        {
            Debugger.LogWarningIfPostProcessingInactive(self);
