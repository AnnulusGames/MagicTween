...
{
    [BurstCompile]
    public readonly unsafe struct ValueAnimationCurve : IDisposable
    {
        readonly UnsafeList<Keyframe> keys;
        ...
        public ValueAnimationCurve(AnimationCurve animationCurve, Allocator allocator)
        {
            keys = new UnsafeList<Keyframe>(animationCurve.keys.Length, allocator);
            fixed (Keyframe* src = &animationCurve.keys[0])
            {
                UnsafeUtility.MemCpy(keys.Ptr, src, animationCurve.keys.Length * sizeof(Keyframe));
            }
            keys.Length = animationCurve.keys.Length;
            keys.Sort(default(KeyframeComparer));
            preWrapMode = animationCurve.preWrapMode;
            postWrapMode = animationCurve.postWrapMode;
        }

        public ValueAnimationCurve(ValueAnimationCurve source, Allocator allocator)
        {
            keys = new UnsafeList<Keyframe>(source.keys.Length, allocator);
            UnsafeUtility.MemCpy(keys.Ptr, source.keys.Ptr, source.keys.Length * sizeof(Keyframe));
            keys.Length = source.keys.Length;
            keys.Sort(default(KeyframeComparer));
        ...
        [BurstCompile]
        float Evaluate(float time, ref Keyframe keyframe, ref Keyframe nextKeyframe)
        {
        ...
        [BurstCompile]
        float CubicBezier(in float4 curveMatrix, float t)
        {
            float tt = t * t;
        ...
        [BurstCompile]
        float CubicBezier(in float3 curveMatrix, float t)
        {
        ...
        [BurstCompile]
        float DeCasteljauBezier(int degree, float4 coords, float t)
        {
            float one_t = 1 - t;
        ...
        [BurstCompile]
        void GetTWithBisectionMethod(float time, in float4 curveXCoords, ref float t, ref float tBottom, ref float tTop, ref float diff)
        {
        ...
        [BurstCompile]
        void GetTWithNewtonMethod(float time, in float4 xCoords, in float4 curveXCoords, ref float t, ref float tBottom, ref float tTop, ref float diff)
        {
            const float accuracy = 0.0000001f;
            const int maxIterationCount = 20;
            int iterationCount = 0;
        ...
        [BurstCompile]
        float UseNewtonMethod(float4 curveCoords, float coord, float t, float3 curvePrimeCoords, float4 primePrimeCoords, out float coordAtT)
        {
        ...
        [BurstCompile]
        void UpdateTLimits(float x, float time, float t, ref float tBottom, ref float tTop)
        {
            if (x > time)
            {
